    ...
    while True:
        ...
        if results.multi_handedness:
            for hand_label in results.multi_handedness:
                hand_jugg=str(hand_label).split('"')[1]#确定左右手
                print(hand_jugg)
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                hand_local = []
                for i in range(21):#遍历出21个特征点
                    x = hand_landmarks.landmark[i].x*frame.shape[1]
                    y = hand_landmarks.landmark[i].y*frame.shape[0]
                    hand_local.append((x,y))#得出21个特征点的数组
                if hand_local:
                    angle_list = hand_angle(hand_local)#得到每个手指信息
                    gesture_str = h_gesture(angle_list)#得到特征信息
                    #print(gesture_str)
                    cv2.putText(frame,gesture_str,(50,100),0,1.3,(0,0,255),2)#显示特征信息
